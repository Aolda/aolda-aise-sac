...
| 파일 | 구현 내용 |
| --- | --- |
| `tasks/1_control_plane.yml` | `k8s_apply_control_plane=true`일 때 Controller Manager 인증/TLS 파라미터를 추가하고, master 주요 설정 파일과 인증서/키 파일 권한을 제한하며 Kubernetes 버전을 출력합니다. |
| `tasks/2_api_server.yml` | `k8s_apply_api_server=true`일 때 API Server 익명 인증 차단, RBAC/Node 인가, 취약한 token auth 제거, Scheduler/Controller Manager bind address 제한, Admission Plugin, TLS, Audit Log 설정을 적용합니다. |
| `tasks/3_etcd.yml` | etcd 저장 데이터 암호화는 수동 승인 항목으로 안내하고, `k8s_apply_etcd_tls=true`와 인증서 precheck 통과 시 API Server와 etcd 간 TLS 및 etcd 서버 TLS/클라이언트 인증 설정을 적용합니다. |
| `tasks/4_kubelet.yml` | `k8s_apply_kubelet=true`일 때 Kubelet 익명 인증 차단, Webhook 인가, kernel 기본값 보호, TLS cipher suite, kubelet 설정/인증서 파일 권한을 적용하고 kubelet 버전을 출력합니다. |
| `tasks/5_policies.yml` | 기본은 PSA/NetworkPolicy 상태 점검이며, `k8s_apply_policies=true`일 때만 PSA 라벨, RBAC/NetworkPolicy/Secret 관리 정책과 안전한 Pod 예시 템플릿을 조건부로 배포합니다. |
...
- 위험 작업은 기본 비활성입니다. control-plane/API Server/etcd TLS/kubelet/policy 적용은 각 `k8s_apply_*` 변수를 명시적으로 켠 경우에만 수행됩니다.
- `k8s_apply_api_server=true`이면 `/etc/kubernetes/manifests` 하위 정적 파드 manifest에 보안 파라미터가 추가 또는 갱신되고 취약한 `--token-auth-file` 설정이 제거됩니다.
- `k8s_apply_control_plane=true`이면 주요 kubeconfig/manifest 파일은 `root:root`, `0644`로 조정되고 인증서 파일은 `0644`, key 파일은 `0600`으로 조정됩니다.
- etcd 저장 데이터 암호화는 자동 완전 적용하지 않습니다. 백업과 기존 Secret 재암호화가 필요한 `manual-required` 항목입니다.
- `k8s_apply_etcd_tls=true`이고 인증서 precheck가 통과하면 API Server와 etcd 사이의 TLS 인자를 설정합니다.
- `k8s_apply_kubelet=true`이면 `/var/lib/kubelet/config.yaml`의 인증/인가/TLS/kernel 보호 설정이 변경됩니다.
- PSA/NetworkPolicy는 기본 check-only입니다. `k8s_apply_policies=true`와 세부 flag를 함께 켠 경우에만 적용 또는 템플릿 생성이 수행됩니다.
- `k8s_enable_rbac_controls`가 `true`이면 제한된 ServiceAccount, Role, RoleBinding이 생성됩니다. 기본값은 `false`입니다.
...
- manifest 변경 후 Kubelet 서비스 재시작은 `k8s_restart_kubelet=true`일 때만 수행됩니다. 기본값은 `false`입니다.
...
| `k8s_node_role` | `auto` | 노드 역할 감지 방식입니다. |
| `k8s_restart_kubelet` | `false` | manifest/config 변경 후 Kubelet 재시작 여부입니다. 운영 영향이 있어 기본 비활성입니다. |
| `k8s_validate_after_apply` | `true` | 적용 후 validate task 실행 여부입니다. |
| `k8s_apply_control_plane` | `false` | Control Plane task 적용 여부입니다. manifest/권한 변경 위험이 있어 기본 비활성입니다. |
| `k8s_apply_api_server` | `false` | API Server task 적용 여부입니다. 정적 파드 manifest 변경 위험이 있어 기본 비활성입니다. |
| `k8s_apply_etcd_tls` | `false` | etcd TLS 설정 적용 여부입니다. 인증서 precheck가 필요해 기본 비활성입니다. |
| `k8s_apply_etcd_encryption_config` | `false` | etcd 저장 데이터 암호화 인자 적용 여부입니다. Secret 재암호화 절차가 필요해 기본 비활성입니다. |
| `k8s_apply_kubelet` | `false` | Kubelet task 적용 여부입니다. 노드 워크로드 영향이 있어 기본 비활성입니다. |
| `k8s_apply_policies` | `false` | PSA/NetworkPolicy/RBAC/템플릿 적용 여부입니다. 워크로드 차단 위험이 있어 기본 비활성입니다. |
| `k8s_check_policies` | `true` | 정책 상태를 자동 변경 없이 점검할지 여부입니다. |
| `k8s_apply_admission_plugins` | `false` | Admission Plugin 적용 여부입니다. Kubernetes 버전 호환성 확인이 필요해 기본 비활성입니다. |
...
| `k8s_min_admission_plugin_kubernetes_minor` | `25` | Admission Plugin 적용을 허용할 최소 Kubernetes minor version입니다. |
...
| `k8s_acknowledge_secret_reencryption_required` | `false` | 기존 Secret 재암호화 절차 필요성을 확인했는지 여부입니다. |
...
| `k8s_apply_psa_labels` | `false` | PSA 라벨 적용 여부입니다. 기본은 점검만 수행합니다. |
...
| `k8s_create_secure_pod_template` | `false` | 안전한 Pod 예시 템플릿 생성 여부입니다. 기본 자동 생성하지 않습니다. |
...
| `k8s_enable_rbac_controls` | `false` | 제한된 ServiceAccount/Role/RoleBinding 생성 여부입니다. 기본 자동 생성하지 않습니다. |
...
| `k8s_enable_secret_controls` | `false` | Secret 관련 템플릿/점검 실행 여부입니다. 기본 자동 생성하지 않습니다. |
