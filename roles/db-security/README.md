...
- DB 계정 관리는 전체 허용 계정 목록을 기준으로 나머지를 삭제하는 방식이 아닙니다.
- 빈 이름의 익명 사용자, `db_test_users`에 지정된 테스트 계정, `db_unnecessary_accounts`와 `db_disallowed_account_hosts`에 운영자가 명시한 계정만 삭제합니다.
- 운영 계정 전체를 사전에 정의하기 어렵기 때문에 알 수 없는 계정은 자동 삭제하지 않고 DB Account Report에 출력합니다.
- 삭제 대상에 운영 계정이 잘못 포함되는 것을 방지하기 위해 기본 삭제 보호 목록과 환경별 추가 삭제 보호 목록을 둡니다.
- 권한 회수 예외 계정은 `db_privilege_exempt_users`로 관리하고, 계정 삭제 보호는 `db_account_delete_protected_users` 계열 변수로 별도 관리합니다.
...
| `db_account_delete_protected_users` | `root`, `mysql`, `debian-sys-maint` | 기본 DB 계정 삭제 보호 목록입니다. 삭제 대상 목록에 포함되면 precheck에서 실패합니다. |
| `db_account_delete_protected_users_extra` | `[]` | 환경별 추가 DB 계정 삭제 보호 목록입니다. OpenStack 서비스, 모니터링, 백업, DBA 계정을 inventory/group_vars에서 추가합니다. |
| `db_include_privilege_exempt_users_in_delete_protection` | `true` | 권한 회수 예외 계정을 삭제 보호 목록에도 포함할지 여부입니다. |
| `db_regular_users` | `[]` | 권한 회수 대상 일반 DB 사용자 목록입니다. 문자열 계정명 또는 `name`/`host` mapping을 사용할 수 있습니다. |
| `db_privilege_exempt_users` | `prometheus`, `mysqld_exporter` | 권한 회수 예외 계정입니다. 삭제 예외와는 역할이 다르며, 기본값에서는 삭제 보호 effective list에도 포함됩니다. |
...
| `db_password_policy_type` | `auto` | 적용할 패스워드 정책 유형입니다. `auto`, `mariadb_simple`, `mysql_validate`를 사용합니다. |
