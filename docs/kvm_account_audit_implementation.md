...
### 목표
KVM 하이퍼바이저 호스트에서 **30일 이상 미사용 KVM 제어 계정**을 자동으로 식별하고, 공통 보안조치 보고서 및 자동화 스크립트를 생성합니다.
...
#### 2️⃣ 공통 보안조치 보고서 생성 (Markdown)
- 다른 role과 동일한 `security_report` callback 보고서 생성
- 파일명 형식: `reports/REPORT.kvm-account-audit.<timestamp>.md`
- 계정 감사 task의 조치 대상 여부와 적용 결과를 role 공통 표 형식으로 집계
...
#### 4️⃣ 조건부 처리
- 미사용 계정 없음 → 계정 조치 및 수동 스크립트 생성 스킵
- 미사용 계정 있음 → 공통 보고서에 task 결과 기록 + 수동 스크립트 생성
...
| 단계 | 작업 | 멱등성 | 비고 |
|------|------|--------|------|
| 1 | 정책 변수 호환성 설정 | ✅ | 기존 변수 구조 지원 |
| 2 | KVM 계정 & 미접속 기간 조회 | ✅ | `lastlog` 활용, bash 스크립트 |
| 3 | 미사용 계정 리스트 파싱 | ✅ | Jinja2 필터 활용 |
| 4 | 잠금/삭제 대상 산정 | ✅ | 허용/제외 계정 반영 |
| 5 | **[조건]** 계정 잠금/삭제 처리 | ✅ | `security_action_mode`와 `kvm_account_action` 기준 |
| 6 | **[조건]** 수동 스크립트 배포 | ✅ | `kvm_account_manual_script_dir`에 생성 |
| 7 | 결과 출력 | ✅ | 공통 callback 보고서에 task 결과 집계 |

### 주요 Ansible 모듈 사용

```yaml
# 1. shell - lastlog 기반 미사용 계정 추출
shell: |
...
user:
  name: "{{ item[0] }}"
  password_lock: true
  shell: "{{ kvm_nologin_shell }}"

# 3. template - 자동 스크립트 생성
template:
  src: apply_account_restrictions.j2
  dest: "{{ kvm_account_manual_script_dir }}/apply_account_restrictions_{{ ansible_date_time.iso8601_basic_short }}.sh"
...
**보고서 위치**:
```
reports/REPORT.kvm-account-audit.20260210_153045.md
```
